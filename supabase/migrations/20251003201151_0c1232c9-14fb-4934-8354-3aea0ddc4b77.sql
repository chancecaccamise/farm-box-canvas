-- Fix populate_weekly_bag_from_template to handle duplicate protein selections correctly
...
-- One-time repair: Fix the current protein pack bag
DO $$
DECLARE
  v_bag_id UUID;
...
BEGIN
  ...
  RAISE NOTICE 'Fixing protein pack bag % with duplicate handling', v_bag_id;
  
  -- Delete old protein items
  DELETE FROM public.weekly_bag_items
  WHERE weekly_bag_id = v_bag_id
    AND item_type = 'user_selected_protein';
  
  -- Repopulate with correct quantities
  PERFORM populate_weekly_bag_from_template(v_bag_id, 'protein-pack', v_week_start);
  
  RAISE NOTICE 'Successfully fixed protein pack duplicates';
