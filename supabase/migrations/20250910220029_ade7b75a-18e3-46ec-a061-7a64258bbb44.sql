-- Add columns to weekly_bags table to store user selections
ALTER TABLE public.weekly_bags 
ADD COLUMN user_protein_selections TEXT[], -- Array of protein IDs selected by user
ADD COLUMN user_carb_selections TEXT[], -- Array of carb IDs selected by user  
ADD COLUMN user_full_farm_bag_protein TEXT, -- Single protein selection for full farm bag
ADD COLUMN user_full_farm_bag_carb TEXT; -- Single carb selection for full farm bag

-- Add a new item_type for user-selected items
ALTER TABLE public.weekly_bag_items 
ADD CONSTRAINT check_item_type CHECK (item_type IN ('box_item', 'addon', 'user_selected_protein', 'user_selected_carb'));

-- Update the populate_weekly_bag_from_template function to handle user selections
...
BEGIN
  ...
  IF box_size_name = 'full_farm_bag' THEN
    -- Full Farm Bag: Add user-selected protein and carb, then fill with vegetables
    ...
    -- Add 6 vegetables from admin template (excluding user-selected items)
    SELECT EXISTS(
    ...
    -- Add vegetables from template (limit to 6, exclude proteins/carbs)
    IF template_week_to_use IS NOT NULL THEN
      FOR template_record IN
        ...
        WHERE bt.week_start_date = template_week_to_use 
          AND bt.box_size = box_size_name
          AND bt.is_confirmed = true
          AND p.category IN ('vegetables', 'herbs', 'fruits')
          AND p.id NOT IN (
